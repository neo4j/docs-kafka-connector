...
[IMPORTANT]
CDC Strategy uses schemas to capture type information and thus requires a message converter with schema support.
...
| neo4j.cdc.topic.\{NAME}.key-strategy | One of `SKIP`, `ELEMENT_ID`, `ENTITY_KEYS`, `WHOLE_VALUE`. Serialisation strategy for CDC topic key.
