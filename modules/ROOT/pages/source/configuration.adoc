...
| neo4j.payload-mode
| Defines the structure of change messages. One of `COMPACT`, `EXTENDED`, `RAW_JSON_STRING`. `COMPACT` provides simpler messages but faces schema compatibility issues if property types change. `EXTENDED` includes type information to avoid such issues. `RAW_JSON_STRING` produces a raw JSON string representation of the data.
...
For guidance on choosing a strategy, see xref:source/best-practices.adoc#cdc-key-strategy[Best practices for Source connector].
