...
Within a single Neo4j transaction, CDC provides a `seq` value for ordering the change records in the CDC output, but the order of those records is not guaranteed to match the order in which the operations were applied during the transaction.

For more information, see link:{page-canonical-root}/cdc/current/procedures/output-schema/#order[Change Data Capture > Format of change events > Order of changes].
...
[NOTE]
====
The CDC Source connector provides JMX metrics to monitor the status of the data ingestion.
See xref:source/monitoring.adoc[Source Monitoring] for more information.
====
