...
[TIP]
====
Beyond schema enforcement, using a compact binary format such as Avro also improves connector throughput compared to JSON.
See xref:source/best-practices.adoc#serialization-format[Best practices for Source connector] for more details.
====
