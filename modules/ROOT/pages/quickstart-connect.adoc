...
The source will retrieve changes for the configured pattern.
These changes are then serialized and published to the configured topic.
...
The property `topic` defines where each message will be pushed.
So given the above configuration, the structure of the message will be the following (serialized according to the selected message format):
...
This will result in new messages being published to the topic named `my-topic`.

=== Sink

One way to consume the messages published above to `my-topic` is to use the sink.
