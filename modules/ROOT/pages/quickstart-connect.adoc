...
Save the following JSON file into a local directory named as `source.avro.neo4j.json`.
...
which in turn will result in there new messages to be published into `my-topic` topic.
...
In this case, we are configuring the SINK instance to consume data in AVRO format.
...
Now you can access your Confluent Control Center instance under: \http://localhost:9021/clusters, and check the created `my-topic` as specified in the connector configuration, as well as the connect cluster to see SINK and SOURCE instances running.

Having both SOURCE and SINK connectors running, we are expecting that the previously created `:TestSource` nodes to trigger there messages in the `my-topic` topic by the SOURCE instance.
