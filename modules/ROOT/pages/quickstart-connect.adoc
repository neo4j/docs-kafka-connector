...
In this quickstart guide, we will use Docker Compose to create an environment with Confluent and Neo4j running inside Docker and then create a source instance which will query changes when nodes of specific label are created in a Neo4j database and publish the changes into a Kafka topic named `my-topic`.
...
If you access your Confluent Control Center instance under http://localhost:9021/clusters, then it should report that the cluster is healthy, with one Broker and connect-default should appear under the Connect instance.
...
Confirm that you can access the Confluent Control Center instance at http://localhost:9021/clusters and that the Cluster reports as healthy (this may take 90-120s). You should have one Broker, several Topics and one Connect in the Control Center

=== Configure SOURCE instance

First up, we need to set up Neo4j as a source database that will provide messages for topics, do do this save the following JSON file into a local directory named as `source.avro.neo4j.json`.

.source.avro.neo4j.json
[source,json]
----
...
In Control Center, confirm that the Neo4jSourceConnectorAVRO source connector has been created in the Connect tab, under connect-default.
...
First, save the following JSON file into a local directory named as `sink.avro.neo4j.json`.

.sink.avro.neo4j.json
[source,json]
----
...
This configures the SINK instance to consume data in AVRO format.
...
In Control Center, you can confirm that `my-topic` appears in Topics, and the Sink instance Neo4jSinkConnector appears under Connect, connect-default.
