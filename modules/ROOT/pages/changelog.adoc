...
Only messages that fail because of their own content are now routed to the dead letter queue.
| When a batch failed and a xref:sink/error-handling.adoc[dead letter queue] was configured, sink connectors reported the offending message to the DLQ regardless of the cause, so a failure unrelated to message content, such as a connectivity problem or an unavailable database, moved messages to the DLQ instead of letting Kafka Connect retry them.
Only data errors are now sent to the DLQ, and any other failure propagates and fails the task so that Kafka Connect can retry it and report it.
Malformed messages in the xref:sink/cud.adoc[CUD], xref:sink/cdc.adoc[CDC], and xref:sink/pattern.adoc[Pattern] strategies are now consistently classified as data errors, so they continue to reach the DLQ as before.
|===
...
Sink connectors no longer reject messages that contain `null` property values.
| Property maps carrying a `null` value were rejected with an `InvalidDataException`, so a xref:sink/cud.adoc[CUD] operation that legitimately removed a field by setting it to `null` failed instead of being applied.
The same validation also affected the property maps of xref:sink/cdc.adoc[CDC] change events.
`null` values are now accepted, while values of an unexpected type are still rejected.
|===
...
Sink connectors now group messages by topic and partition.
| Sink connectors grouped incoming messages by topic only, so a single batch could span multiple partitions of the same topic.
Since the offset tracked for exactly-once semantics (xref:sink/configuration.adoc[`neo4j.eos-offset-label`]) is derived per topic and partition, batches spanning partitions could record an offset against the wrong partition, which in turn could cause messages to be skipped or applied more than once.
Messages are now grouped by both topic and partition, and each partition maintains its own offset.

a|
label:bug[]
label:fixed[]

Message contents are no longer written to logs when a batch fails.
| When a sink connector failed to process a batch, the first and last records of that batch were logged in full, including their keys and values.
Only message metadata (topic, partition, offset, and timestamp) is now logged.
|===
...
Support for dynamic labels and relationship types in a `MATCH` clause removed from Sink connectors.
| The Sink connector uses entity lookups with dynamic node labels and relationship types when the feature is available in the target database.
However, the Cypher planner can potentially produce a slow plan when the matched relationships include several properties.
The support for dynamic labels and relationship types in a `MATCH` clause has been removed until a more consistent behaviour can be guaranteed.
|===
...
New JMX metric `last_cdc_tx_commit_age` for CDC Source and Sink connectors.
| Reports the number of seconds since the last committed CDC message was polled (Source) or pushed (Sink) by the connector. Returns `-1` if no commit has been processed yet.

a|
label:feature[]
label:new[]

CUD sink strategy now supports Exactly Once semantics and batched processing.
| The CUD sink strategy has been re-implemented on top of the shared sink action abstraction, so it now supports Exactly Once offset tracking and benefits from the batched processing strategies introduced for CDC sink handlers.

a|
label:feature[]
label:new[]

Pattern sink handlers now support Exactly Once semantics and batched processing.
| Node and relationship pattern sink handlers have been re-implemented on top of the shared sink action abstraction, so they now support Exactly Once offset tracking and benefit from the batched processing strategies introduced for CDC sink handlers.

a|
label:bug[]
label:fixed[]

Fix last database transaction ID query to handle multiple results.
| In clustered Neo4j environments, the query used by the CDC Source connector to fetch the last database transaction ID could return multiple values, causing the connector to fail. The query has been updated to correctly handle multiple results.
...
JMX metrics for CDC Source and Sink connectors.
| The connector now provides JMX metrics for the Change Data Capture (CDC) strategy to monitor data ingestion progress.

a|
label:feature[]
label:new[]

Ability to prefer leader for querying CDC events in Source connector.
| The CDC Source connector now includes a configuration option `neo4j.cdc.use-leader` to prefer querying the leader instance for CDC events, which can help reduce replication lag and improve the freshness of the data being captured.

a|
label:feature[]
label:new[]

Exactly Once support for CDC Sink connector.
| The CDC Sink connector can now be configured to support Exactly Once semantics by tracking applied offsets in the target database.
