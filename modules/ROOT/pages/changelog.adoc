...
Exactly Once support for CUD sink strategy.
| The CUD sink strategy can now be configured to support Exactly Once semantics by tracking applied offsets in the target database, aligning its behavior with the CDC sink handlers.
...
| In clustered Neo4j environments, the query used by the CDC Source connector to fetch the last database transaction ID could return multiple values, causing the connector to fail. The query is now updated to correctly handle multiple results.
