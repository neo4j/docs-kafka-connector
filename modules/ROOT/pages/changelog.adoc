...
CDC Source connector declare itself as Exactly Once capable.
| For environments that support link:{https://cwiki.apache.org/confluence/display/KAFKA/KIP-618%3A+Exactly-Once+Support+for+Source+Connectors}[KIP-618: Exactly Once semantics for Source Connectors], CDC source connector will take advantage of this feature without any additional configuration.
...
| New set of CDC sink handlers are introduced to provide better performance when processing CDC events in sink connector.
These handlers will process messages in batches and apply them into the database in a more efficient way.
New handlers utilize APOC Core `apoc.cypher.doIt` procedure and Cypher Set & Remove clauses with dynamic labels, and will be preferred over the default handlers based on availability of these features.
