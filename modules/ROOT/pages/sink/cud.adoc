...
Starting from version 5.3.1, a batched sink handler is available for the CUD strategy to provide better throughput when processing CUD events.
...
Starting from version 5.3.1, the CUD sink handler can be configured to achieve exactly-once processing guarantees by keeping track of the last successfully processed message offset in the database.
