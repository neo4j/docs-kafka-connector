...
For instance, if you have two topics, `topic.1` and `topic.2` that you publish CUD formatted messages, you can configure the sink instance as follows.
...
Node format defines a set of fields which describes the operation (`create`, `update`, `merge` and `delete`), label and properties of a node entity.
...
[NOTE]
Delete messages are for **individual nodes** it’s not intended to be a generic way of doing cypher query building from JSON
