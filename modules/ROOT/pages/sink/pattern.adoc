...
Starting from version 5.3.1, batched sink handlers are available for the Pattern strategy to provide better throughput when processing pattern events.
...
Starting from version 5.3.1, pattern sink handlers can be configured to achieve exactly-once processing guarantees by keeping track of the last successfully processed message offset in the database.
