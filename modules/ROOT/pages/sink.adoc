...
| Name | Description
| neo4j.topic.pattern.node.\{NAME} | Node pattern to apply on the messages received from the specified topic.
Example: `"neo4j.topic.pattern.node.user": "(:User\{!userId})"`
| neo4j.topic.pattern.relationship.\{NAME} | Relationship pattern to apply on the messages received from the specified topic.
Example: `"neo4j.topic.pattern.node.user": "(:User\{!userId})-[:KNOWS]->(:User\{!otherUserId})"`
