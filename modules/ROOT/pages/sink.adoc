...
`bootstrap.servers` property is required by the Kafka Connect Framework.
The latter provides support only for deserialization errors and message transformations (please look into the following link for further details: {url-confluent-blog}/kafka-connect-deep-dive-error-handling-dead-letter-queues/).
In order to cover all failure modes, the broker locations are required until link:https://issues.apache.org/jira/browse/KAFKA-8597[this issue] is addressed.
...
| `neo4j.batch-timeout | Batch transaction timeout duration in seconds, minutes, hours or days. Examples: `1d`, `2h`, `3m`, `4s`, `5ms` (Default: `0s`)
...
| Name | Description
| neo4j.topic.cypher.\{NAME} | Cypher statement to run for the specified topic. Example: `neo4j.topic.cypher.my-topic=MERGE (n:Person {name: event.name})`
|===
