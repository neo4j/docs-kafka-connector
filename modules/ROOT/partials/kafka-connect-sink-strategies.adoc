...
Be aware that by default this strategy will overwrite node properties. If you need a more flexible approach, where nodes properties has to be merged, then you have to add the following parameter in your connector configuration:

[source,json,subs="verbatim,attributes"]
----
"{environment}.topic.pattern.merge.node.properties.enabled": "true"
----
...
in this last case we assume that `User` is the source node and `Product` the target node
...
Similar to the `Node pattern` strategy, be aware that by default this strategy will overwrite relationship properties. Again, if you need a more flexible approach, where relationship properties has to be merged, then you have to add the following parameter in your connector configuration:

[source,json,subs="verbatim,attributes"]
----
"{environment}.topic.pattern.merge.relationship.properties.enabled": "true"
----
